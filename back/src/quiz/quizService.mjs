...
export class quizService {
    static async addOrSetQuiz({ userId, idx }){
        ...
        try{
            if(questions.length <= idx){
                ...
                return { status: 400, message: "400 Error: 퀴즈 배열 데이터의 길이보다 크거나 같은 인덱스는 가질 수 없습니다."};
            }
            if(userQuiz === null){
    ...
    static async getQuiz({ userId }){
        ...
        if(questions.length <= idx){
            return { status: 500, message: "500 Error: mongoDB에 저장된 userQuiz의 인덱스가 퀴즈 배열 데이터 길이를 넘습니다.(잘못된 저장)"};
        } 
